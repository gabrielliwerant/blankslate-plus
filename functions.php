...
function mysalesforcecrm_avatar()
{
...
function get_post_slug()
{
	...
	{
		$post_parent = get_post($current_post->post_parent);

		return $post_parent->post_name;
	}
}
 ...
 * @param type $html_id 
 */
function mysalesforcecrm_content_nav( $html_id ) {
	global $wp_query;

	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo esc_attr( $html_id ); ?>">
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'mysalesforcecrm' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'mysalesforcecrm' ) ); ?></div>
		</nav>
	<?php endif;
}
//add_filter( "mysalesforcecrm_content_nav" );
...
function blankslate_custom_comments($comment, $args, $depth)
{
	...
	<li id="comment-<?php comment_ID() ?>" <?php comment_class() ?>>
		...
		<div class="left">
			<div class="comment-avatar"><?php mysalesforcecrm_avatar() ?></div>
		</div>
		<div class="left">
